...
class EstateProperty(models.Model):
    ...
    seller_id = fields.Many2one('res.partner', string="Salesman", index=True,
                                readonly=True, default=lambda self: self.env.user.partner_id.id)
    new_seller_id=fields.Many2one('res.users', string="Salesman", index=True,
                                  readonly=True, default=lambda self: self.env.user.partner_id.id)
